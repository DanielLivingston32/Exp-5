...
document.getElementById('flip').addEventListener('click', (e) => {
  if (Math.random() > 0.5) {
    document.getElementById('coin-img').setAttribute('src', 'heads.jpg');
    document.getElementById('heads-total').innerText = ++heads;
  } else {
    document.getElementById('coin-img').setAttribute('src', 'tails.png');
...
document.getElementById('submit').addEventListener('click', (e) => {
  ...
  const one = 100 + (200 / 100) * 9;
  const two = 100 + (400 / 100) * 9;
